...
declare
  ...
  requested_unit_key text := regexp_replace(
    lower(regexp_replace(regexp_replace(coalesce(requested_unit, ''), '^\s*unit\s+', '', 'i'), '(^|[^a-zA-Z0-9])0+([0-9])', '\1\2', 'g')),
    '[^a-z0-9]+',
    '',
    'g'
  );
  ...
  if requested_unit is not null then
    ...
    where property_id in (linked_property_id, requester.property_manager_property_id)
      and (
        ...
        or regexp_replace(
          lower(regexp_replace(regexp_replace(coalesce(unit_name, ''), '^\s*unit\s+', '', 'i'), '(^|[^a-zA-Z0-9])0+([0-9])', '\1\2', 'g')),
          '[^a-z0-9]+',
          '',
          'g'
        ) = requested_unit_key
