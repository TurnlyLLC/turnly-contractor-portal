...
function passwordPromptMarkup(message = "", error = false) {
  return `
    <div class="access-notice contractor-password-change">
      ...
      <form id="forcedPasswordChangeForm" class="forced-password-form">
        <label>
          ...
          <input id="forcedNewPassword" type="password" autocomplete="new-password" minlength="8" required />
        </label>
        <label>
          <span>Confirm password</span>
          <input id="forcedConfirmPassword" type="password" autocomplete="new-password" minlength="8" required />
        </label>
        <p id="forcedPasswordMessage" class="${error ? "error" : ""}" aria-live="polite">${message}</p>
...
async function renderPasswordChangeRequired(user) {
  ...
  const portalRoot = document.getElementById("contractorPortalApp");
  if (portalRoot) portalRoot.innerHTML = passwordPromptMarkup();
...
async function loadContractorPortal() {
  await import("./contractor-portal.js?v=20260724-force-password-change");
  await import("./contractor-job-flow-mobile.js?v=20260729-checklist-scroll-top");
}
